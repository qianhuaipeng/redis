...
public class RedisClient {
    ...
    static {
        ...
        config.setMaxWaitMillis(1000);//获取连接时最大的等待时间。如果超时就票抛出异常
        config.setTestOnBorrow(false);//在borrow一个redis实例时，是否提前进行validate操作；如果为true则得到的redis实例是可用的
        config.setTestOnReturn(true);
        System.out.println("======init=====");
        jedisPool = new JedisPool(config,HOST,PORT,WAIT_TIMEOUT);
    }
    ...
    public static void string() throws Exception{
        ...
        //jedis.incr("test");
        jedis.append("test","allasdad");
        System.out.println(jedis.get("test"));

        String type = jedis.type("test");
        System.out.println("type: " + type);

        long length = jedis.strlen("test");
        System.out.println("length: " + length);

        jedis.set("testString", "testString");
    ...
    public static void set(){
        ...
        long length = jedis.scard("testSet");
        System.out.println("testSet集合的长度：" + length);
        System.out.println();

        jedis.srem("testSet","wch");
